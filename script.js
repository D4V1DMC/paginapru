...
fruits.forEach(fruit => {
    fruit.addEventListener("mouseover", () => {
        fruit.textContent = "💥"; // explosión divertida
        setTimeout(() => {
            // vuelve al original
...
// Mensajes según el número de clicks
const heartMessages = {
    ...
    200: "¡Ups! Sé que a veces desaparezco… pero gracias por seguir jugando conmigo y aguantandome y ahora si esto era lo ultimo💙",
};
...
icon.addEventListener("click", () => {
    ...
    // Reinicia después de 200 para poder volver a disparar
    if (heartClicks >= 300) heartClicks = 0;
});

// Función para mostrar mensaje animado tipo burbuja
...
// --- Definir los "pi" con sus tiempos y duración ---
// startTime en segundos, duration en segundos
showPi("'pi'", 51, 0.5);    // aparece a los 5s, dura 1s
showPi("'pi'", 51.5, 0.5);
showPi("'pi'", 52, 0.5);
showPi("'pi'", 52.5, 0.3);

showPi("'pi'", 67, 0.5);    // aparece a los 5s, dura 1s
showPi("'pi'", 67.5, 0.5);
showPi("'pi'", 68, 0.5);
showPi("'pi'", 68.5, 0.3);

showPi("'pi'", 116.5, 0.5);    // aparece a los 5s, dura 1s
showPi("'pi'", 117, 0.5);
showPi("'pi'", 117.5, 0.5);
showPi("'pi'", 118, 0.3);

showPi("'pi'", 133, 0.5);    // aparece a los 5s, dura 1s
showPi("'pi'", 133.5, 0.5);
showPi("'pi'", 134, 0.5);
showPi("'pi'", 134.5, 0.3);

showPi("'pi'", 166, 0.5);    // aparece a los 5s, dura 1s
showPi("'pi'", 166.5, 0.5);
showPi("'pi'", 167, 0.5);
showPi("'pi'", 167.5, 0.3);
